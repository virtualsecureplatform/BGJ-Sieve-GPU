...
prep_export=(--export=ALL)
if [[ -n "${RESUME_CSD:-}" ]]; then
    prep_export=(--export="ALL,RESUME_CSD=${RESUME_CSD}")
fi
prep_job="$(sbatch --parsable "${prep_export[@]}" reproduce/svp164-seed0/preprocess_bgj_bkz138_a100x4.sbatch)"
